...
const ShowUsers = () => {
  ...
  const {Members, searchmember, SearchMembers} = useBookContext();
  ...
  return (
    <div className="p-4 sm:ml-64"> 
      ...
      <div className="flex my-5 justify-end gap-2 items-center">
          ...
          <div className="overflow-x-scroll">
            <table style={{width : "-webkit-fill-available"}}>
              ...
              <tbody className="items-center">
                {Members.length > 0 &&
                  (searchterm.length===0?Members:SearchMembers)?.slice(0, visible)?.map((member, index) => {
          ...
          style={{
            display: show && (searchterm.length===0?Members:SearchMembers).length > 10 ? "flex" : "none",
          }}
        >
